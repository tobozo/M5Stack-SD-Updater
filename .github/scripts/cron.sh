...
git remote add origin https://$GITHUB_ACTOR:$GHTOKEN@github.com/$CLONE_REPOSITORY.git
git config --global user.name $GITHUB_ACTOR
git config --global user.email $GITHUB_ACTOR@users.noreply.github.com

nextAW=`curl -L --silent https://phpsecu.re/api/registry/workflow | jq -r .need_workflow[0]`
...
if [[ $allComments == *"$commentMessage"* ]]; then
  ...
  curlAuth="Authorization: token $GHTOKEN"
