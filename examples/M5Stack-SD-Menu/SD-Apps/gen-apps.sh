...
for D in *; do
  if [ -d "${D}" ]; then
    ...
    if (( $m5enabled == 1 )); then
      ...
      arduino  --pref "build.path=${TRAVIS_BUILD_DIR}/build sketchbook.path=${SDAPP_FOLDER}/${D}" --verbose-build --verify --preserve-temp-files --board $BOARD $PATH_TO_INO_FILE >> $SDAPP_FOLDER/out.log;
